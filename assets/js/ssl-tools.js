...
function sslBuildAuditHtml(data) {
    ...
    else if (!chain.chain_valid) { overallType = 'warning'; overallText = 'Chain Issues Detected'; }
    else if ((tls.security_notes || []).length > 0 && !tls.security_notes[0]?.startsWith('Excellent')) { overallType = 'warning'; overallText = 'Security Warnings Found'; }
            ...
            <div class="ssl-audit-section">
                ...
                <div class="ssl-audit-section-body">
                    <div class="ssl-info-section ssl-info-full">
                        ...
                        ${sslInfoRow('Valid To', sslFormatDate(cert.valid_to), { class: cert.days_left < 30 ? 'ssl-warn-text' : '' })}
                        ${sslInfoRow('Days Left', cert.days_left + (cert.days_left < 30 ? ' ⚠️' : ''), { class: cert.days_left < 30 ? 'ssl-warn-text' : '' })}
    ...
    if (chain && chain.chain) {
        ...
        if (chain.chain_valid) { chainType = 'success'; chainText = 'Valid'; }
        else { chainType = 'danger'; chainText = 'Invalid'; }
                ...
                <div class="ssl-audit-section-body">
                    ...
                    <div class="ssl-chain-visual">
                        <div class="ssl-chain-list">
                            ${chain.chain.map((c, i) => `
                                <div class="ssl-chain-cert ssl-chain-${i === 0 ? 'leaf' : i === chain.chain.length - 1 ? 'root' : 'intermediate'}${c.is_valid ? '' : ' ssl-chain-invalid'}">
                                    <div class="ssl-chain-connector">${i > 0 ? '<i class="fas fa-arrow-down"></i>' : ''}</div>
                                    <div class="ssl-chain-card">
                                        <div class="ssl-chain-badge">
                                            ${i === 0 ? 'Leaf (Server)' : i === chain.chain.length - 1 ? 'Root CA' : 'Intermediate CA'}
                                        </div>
    ...
    // ── TLS Section
    if (tls && tls.versions) {
        const tlsOk = tls.security_notes?.[0]?.startsWith('Excellent');
        html += `
            <div class="ssl-audit-section">
                        ...
                        ${tls.versions.map(v => `
                            <div class="ssl-tls-card ${v.supported ? 'ssl-tls-supported' : 'ssl-tls-unsupported'}">
                                <div class="ssl-tls-header">
                                    <span class="ssl-tls-version">${v.label}</span>
                                    ${v.supported
                ...
                <div class="ssl-audit-section-body">
                    <div class="ssl-hsts-grade">
                        <div class="ssl-grade-circle" style="border-color: ${gradeColor}; color: ${gradeColor};">
                            <span class="ssl-grade-letter">${hsts.grade}</span>
                        </div>
                        <div class="ssl-grade-info">
                            <span class="ssl-grade-score">Score: ${hsts.score}/80</span>
    ...
    async function doGenerate() {
        ...
        try {
            ...
            if (sans.length) {
                ...
                const extAttr = sslAsn1Sequence(extReqOid + sslAsn1Set(sanSeq));
